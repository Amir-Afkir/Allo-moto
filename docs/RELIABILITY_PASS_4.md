...
Le tunnel consulte les cookies même si sessionStorage est vide ou inaccessible. Le choix d’un reçu dans la liste utilise une navigation complète vers son URL explicite pour ne pas réutiliser une transition client de la demande précédente ; les liens entre étapes du dossier restent sans rechargement. `/reserver?stage=confirmed` propose les demandes récentes du navigateur ; un lien avec `reservationId` ouvre le reçu correspondant. Le reçu comporte les dates et montants persistés et se rafraîchit au retour dans l’onglet et toutes les 30 secondes lorsque la page est visible.
